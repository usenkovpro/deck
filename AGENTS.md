...
Manifest, icons and service worker are in and verified working: the worker registers
and activates, and precaches the four pages plus the icons and manifest.

Still to do:

1. **Prove it offline.** Stop the preview server and reload — the app should still
   come up. Not yet tested.
2. **The hashed JS and CSS in `_astro/` are not precached**, only cached on first
   fetch. So a brand-new install that goes offline before its second visit would get
   the HTML with no styles or behaviour. Either precache them, or accept it and say
   so.
3. **Deploy it**, so it can be installed on a phone rather than only on a laptop.
4. Add a short "how to install" note to Setup — the steps differ on iOS and Android
   and are not discoverable.

Icons are generated by `npm run icons` and committed. Only rerun it if the mark
changes.
